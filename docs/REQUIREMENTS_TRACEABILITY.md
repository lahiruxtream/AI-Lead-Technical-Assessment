...
|---|---|---|
| Streamlit chat, multi-turn, streaming, activity panel | `ui/streamlit_app.py`, `/v1/chat/stream` | Implemented with SSE token and lifecycle events |
...
| RLM / recursive exploration | `research_node` in `app/graph.py` | Bounded evidence partitioning, concurrent batch analysis, aggregation |
| Dense + BM25 hybrid search and ranking | `app/retrieval.py` | Pinecone path plus deterministic offline dense fallback; weighted fusion |
...
| Knowledge, MCP, Python tools | `app/tools.py`, `app/mcp_server.py` | Implemented; MCP-like dummy enterprise service is deliberately low-priority/read-only |
| LLM and rationale | `app/llm.py`, `docs/ARCHITECTURE.md` | OpenAI when configured; grounded extractive fallback otherwise |
| LangSmith observability | LangGraph run config and `.env.example` | Automatic graph/model traces when `LANGCHAIN_TRACING_V2` and API key are set |
...
- The local feature-hash vector is a deterministic availability fallback, not a substitute for production semantic embeddings.
